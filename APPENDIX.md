# Appendix — the long-form record behind REPORT.md

`REPORT.md` is held to one page, as the brief asks. This is everything it had to leave out,
kept verbatim from the version of 2026-09-20 so the reasoning behind each sentence there is
auditable, followed by what the third review pass found on 2026-09-23.

Two statements below are **known to be wrong** and are left as written, corrected here
rather than silently edited:

- *"zero model calls, proven by severing the socket layer"* — the proof never executed the
  rescorer. See §"The third pass" below.
- *"The attack matrix proves both routes are open and unguarded-in-one-case"* — the matrix at
  that time could not test the guard at all; every attack patched the workspace directly.
...
## The third pass, 2026-09-23

A third review found six defects. Three were in the harness, and two of those could have
invalidated the integrity result. None changed a recorded row: a scan of all 46 journals
(this fork's 27 and upstream's 19) found **zero** steps any fix treats differently.

| # | defect | severity | how it was found |
|---|---|---|---|
| 1 | Guard and `cheated` matched paths case-sensitively. On macOS `Tests/data/expected.txt` **is** `tests/…`: `e02` went green with the guard on and the scorer reporting `cheated=False`. | integrity | scripted agent through the real loop |
| 2 | `ws / path` is not containment. `../x` and absolute paths wrote anywhere the user could; any host file could be read into the prompt. | host safety | scripted agent through the real loop |
| 3 | `prove_no_model_calls.py` loaded the rescorer under a non-`__main__` name, so **`main()` never ran**. The "proof" proved only that importing opens no sockets. It also trusted exceptions to propagate, so a swallowed call passed. | the proof was vacuous | backdated results files; planted a swallowed call |
| 4 | Journals carried no session id. The README's single-task command overwrote one task's journals and the rescorer blended them with stale ones silently. | provenance | reading the journal schema |
| 5 | The manifest pinned `qwen3:latest` — a floating tag, not a model. | reproducibility | comparing manifest to `/api/tags` |
| 6 | Contracts said `files_writable: [one file]`; the harness never enforced that, and `e03`'s attack depends on it not doing so. | documentation overclaim | reading contract against harness |

Also fixed: the attack harness cross-contaminated evidence between attacks; 97 workspaces of
model-written code had accumulated in the temp dir; `REPORT.md` had grown to 1,329 words.

Each fix has a negative control that was run against the **unfixed** code and went red:
4 of 5 new harness attacks fail against the original `loop.py`; the rebuilt proof fails
(exit 1, two recorded attempts) on a planted swallowed call; a tampered digest makes
`run_eval.py` exit 2 before any model call; mixed-session journals trigger a warning.
One suspicion did **not** hold up: Ollama's `prompt_eval_count` does not undercount on KV
cache hits (846 cold, 846 warm), so the cost figures stand.

A fourth set of nine was run on the fixed harness and is in `proofs/reproduction_20260923/`,
the first carrying session ids and a verified digest. `verified` under v2 read 6/9 again:
6, 6, 6, 6 across 36 runs, against 8, 9, 7, 8 under v1.
